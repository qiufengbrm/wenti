...
- 教程发布、消息通知与特长词云
- 活动资料、文件夹及文件管理
- GLM 直接读取 PDF/图片赛事材料、AI 核对、时间段整理与 Excel 排班收集表导出
- 桌面端、移动端和深色模式适配
...
赛事排班表生成器把原始 PDF 或图片直接交给 GLM-5.3-Flash，以保留表格版面和场次关系。配置 `AI_SETTINGS_ENCRYPTION_KEY` 后，超级管理员可在“系统设置”中保存全站共享的 GLM API Key。完整说明见 [赛事排班表生成器](./README_FULL.md#赛事排班表生成器)。

...
- 从侧栏“工具栏”进入赛事排班表生成器，上传赛事 PDF 或截图，由 GLM 直接读取原文件，经人工核对和时间段调整后导出跟进人员留空的 Excel 收集表。
...
- 从侧栏“工具栏”使用赛事排班表生成器；结果不会自动关联志愿者课表，也不会自动决定跟进人员。
...
- 在“系统设置”维护赛事工具共用的 GLM API Key；密钥经 AES-256-GCM 加密保存，普通管理员和志愿者只能通过后端赛事工具使用，无法读取明文。
...
- 部门负责人可见：首页、志愿者管理、特长词云、课表管理、资料中心、志愿时长管理、教程管理、工具栏、个人信息。
- 部门负责人隐藏分组：任务管理。
- 志愿者可见：首页、申请志愿时长、我的课表、特长词云、资料中心、教程中心、工具栏、个人信息。
- 志愿者隐藏分组：任务广场、我的志愿时长。
...
| 排班表 Excel 导出 | ExcelJS 4.4 |
| 赛事文件理解 | GLM-5.3-Flash 原生文件与视觉输入 |
| 赛事结构化提取 | GLM-5.3-Flash（后端 API） |
...
| 赛事识别资料 | 20 MB/个；最多 8 个且总计 80 MB/次 | 仅接受 PDF、PNG、JPG、JPEG；系统临时目录处理后立即清理 |

Nginx 的 `client_max_body_size` 应略高于系统最大单文件限制，本文示例使用 `520m`。

## 赛事排班表生成器

管理员与志愿者都可以从“工具栏”进入。完整流程为：上传材料 → GLM-5.3-Flash 直接理解原始文件与表格版面 → 提取结构化赛事 → 人工核对 → 按时间间隔分组并人工调整 → 预览并导出 Excel。工具不会自动安排人员，也不会读取或联动课表。

原始 PDF 以 GLM 文件输入、PNG/JPG 以视觉输入直接提交，不再先拆成 OCR 文本块，因此能够结合表头、合并单元格、行列位置和跨页上下文判断时间与场次。支持单个文件最大 20MB、最多 8 个文件、单次总计 50MB；文件只用于当前请求，不写入业务数据库。

GLM 调用只发生在后端，system prompt 与 JSON Schema 位于 `src/services/ai/event-extraction-prompt.ts`，调用和 Zod 复核位于 `src/services/ai/glm-event-extraction.ts`。超级管理员在“系统设置”中保存的 GLM API Key 使用 AES-256-GCM 加密后写入数据库，所有赛事工具用户共享使用；浏览器只能看到掩码。若数据库尚未配置，系统才回退到可选的 `GLM_API_KEY` 环境变量。分组默认把同一天相邻开始时间不超过 30 分钟的赛事放进同组，结束时间为最后一场开始时间加 30 分钟；两项参数都可在页面修改。结束时间仅是排班参考时间。

生成的 `.xlsx` 使用 ExcelJS，包含加粗表头、合理列宽、自动换行、垂直居中、冻结首行和空白“跟进人员”列。当前任务数据只保存在浏览器操作流程中，不写数据库。

本地验收步骤：

1. 配置数据库与 `AI_SETTINGS_ENCRYPTION_KEY`，执行数据库迁移并启动 `npm run dev`。
2. 以超级管理员登录，在“系统设置”保存共享 GLM API Key；再以任意管理员或志愿者账号从侧栏进入“工具栏”。
3. 上传包含明确赛事日期、时间和项目的 PDF/JPG/PNG，等待 GLM 完成原文件分析。
4. 修改缺失项，验证新增、删除、来源展开和重复提示。
5. 调整分组参数，验证合并、拆分、移动赛事和时间段编辑。
6. 导出 Excel，检查换行、冻结表头和空白人员列。
7. 另用错误类型、超过 20 MB 的文件、模糊时间材料验证错误提示与数据保留。
...
# 赛事排班表生成器
# 运行一次 `openssl rand -base64 32`，把输出固定保存在服务器配置中；更换后旧密文将无法解密。
AI_SETTINGS_ENCRYPTION_KEY="replace-with-32-byte-base64-key"

# 可选后备配置；只有管理面板没有保存密钥时才使用。
GLM_API_KEY="your-zhipu-api-key"
GLM_API_URL="https://open.bigmodel.cn/api/paas/v4/chat/completions"
GLM_MODEL="glm-5.3-flash"
GLM_TIMEOUT_MS="135000"
```
...
| `AI_SETTINGS_ENCRYPTION_KEY` | 使用管理面板保存 AI 密钥时必需 | 32 字节 Base64 或 64 位十六进制主密钥；必须稳定保存，不得提交 Git |
| `GLM_API_KEY` | 否 | 管理面板尚未配置时使用的智谱 API Key 后备值 |
| `GLM_API_URL` | 否 | GLM OpenAI 兼容接口地址，默认智谱 `/chat/completions` |
| `GLM_MODEL` | 否 | 赛事提取模型，默认 `glm-5.3-flash` |
| `GLM_TIMEOUT_MS` | 否 | 单次 GLM 请求超时，默认 135000 毫秒；超时后会自动改用兼容 JSON 模式重试一次 |
